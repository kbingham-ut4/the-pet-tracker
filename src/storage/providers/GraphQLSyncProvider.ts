...
interface GraphQLQuery {
  getItem: StorageItem<any> | null;
  getItems: StorageItem<any>[];
}
...
export class GraphQLSyncProvider implements CloudSyncProvider {
  ...
  private async makeGraphQLRequest<T = any>(query: string, variables?: any): Promise<GraphQLResponse<T>> {
    if (!this.initialized) {
  ...
  async downloadItem<T>(key: string): Promise<StorageItem<T> | null> {
    ...
    try {
      const response = await this.makeGraphQLRequest<GraphQLQuery>(query, variables);
