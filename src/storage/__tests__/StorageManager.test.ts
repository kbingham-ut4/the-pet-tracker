...
import { StorageConfig, StorageItem as _StorageItem } from '../types';
...
describe('PetTrackerStorageManager', () => {
  ...
  describe('basic storage operations', () => {
    ...
    it('should retrieve an item successfully', async () => {
      ...
      expect(mockAsyncStorage.getItem).toHaveBeenCalledWith(`pet-tracker-test:pets:${mockPet.id}`);
    });
  ...
  describe('query operations', () => {
    beforeEach(async () => {
      ...
      const _strippedKeys = petsData.map(pet => `pets:${pet.id}`);
