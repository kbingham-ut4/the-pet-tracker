...
export class PetTrackerStorageManager implements OfflineStorageManager {
  ...
  async sync(): Promise<SyncResult> {
    ...
    try {
      ...
      const pendingItems = await this.query<unknown>({ syncStatus: 'pending' });
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      ...
      const allItems = await this.query<unknown>({});
      const keysToRemove: string[] = [];

      for (const [_key, _data] of allItems) {
        const item = await this.localProvider.getItem(_key);
  ...
  async getStats(): Promise<StorageStats> {
    try {
      ...
      for (const [_key, item] of allItems) {
        if (item) {
          if (item.metadata.syncStatus === 'pending') {
            pendingSync++;
          }
          if (item.metadata.syncStatus === 'error') {
            syncErrors++;
          }
