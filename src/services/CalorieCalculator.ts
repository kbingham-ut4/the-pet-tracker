...
export abstract class CalorieCalculator {
  ...
  abstract calculateMaintenanceCalories(
    ...
    _isSpayedNeutered: boolean
  ): number;

  abstract calculateTargetCalories(
    _maintenanceCalories: number,
    _weightGoal: 'maintain' | 'lose' | 'gain'
  ): number;

  abstract getRecommendedWeightRange(_breed?: string): { min: number; max: number } | null;
}
